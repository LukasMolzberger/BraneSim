...
def main() -> None:
    ...
    a_phys = mapper.to_phys_length(a).detach().cpu().numpy()
    ...
    create_3d_animation(
        ...
        title_template="Electron initial state (static, a) - t = {:.3f} fs",
        fps=30,
        dpi=120,
    ...
    x4_disp = disp[:, 3]
    x4_phys = mapper.to_phys_length(x4_disp).detach().cpu().numpy()
