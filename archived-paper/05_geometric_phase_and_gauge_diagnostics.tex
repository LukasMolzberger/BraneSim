...
The gauge-invariant diagnostic for numerical experiments is holonomy computed from overlap products along discretized loops: for the Abelian case, $\gamma_\Gamma = \arg\!\big(\prod_k \langle u(x_k)|u(x_{k+1})\rangle/|\cdots|\big)$; for the non-Abelian case, the holonomy matrix $U_\Gamma = \prod_k U_k$ where each $U_k$ is the unitary part of the overlap matrix $\mathbf{E}(x_k)^\dagger\mathbf{E}(x_{k+1})$.
Gauge invariance of all reported invariants must be verified by applying arbitrary local frame rotations and confirming that $\mathrm{spec}(U_\Gamma)$ and $\gamma_\Gamma$ are unchanged.
...
The $U(1)$-trace / $SU(3)$-traceless channel decomposition (diagnostic D6) applied to the same preliminary run is shown in \Cref{fig:color-channels}.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{diagrams/exp_relaxed_color_channels}
  \caption{$U(1)$-trace / $SU(3)$-traceless channel decomposition (diagnostic D6) of a non-converged rotating-frame-periodic JFNK transient (residual per DOF $\approx 1.9\times10^{-4}$).
  The $SU(3)$ traceless channel carries ${\approx}0.2\%$ of the total energy.
  This is a diagnostic reading on a preliminary run, not a demonstrated dynamical coexcitation or binding; a converged eigenstate has not been obtained.}
