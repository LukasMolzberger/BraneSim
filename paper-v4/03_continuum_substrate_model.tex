...
For $\lambda\gg a$ (small $|k|$ inside the Brillouin zone), the dispersion can be expanded and the leading-order
dynamics can be organized by an isotropic continuum elasticity model with effective Lam\'e-type parameters.
This motivates using an isotropic hyperelastic action as the continuum baseline while keeping the lattice as the
microphysical hypothesis.
...
must inherit a universal effective metric, with residual anisotropy pushed into higher-order corrections.
