...
This paper develops a deterministic substrate theory fragment with a narrow purpose:
...
A cubic lattice of nodes/cells embedded in $\R^4$ is taken as the microphysical substrate.
For wavelengths large compared to the lattice spacing, its dynamics admits a continuum approximation on an intrinsic
$3$-domain $\Omega\subset\R^3$ (\Cref{sec:continuum-model}). An isotropic hyperelastic action remains the analytic
...
\item \textbf{An ordered narrowband carrier sector.}
We assume the substrate admits an ordered, narrowband carrier sector that provides phase rigidity and enables a
controlled multiscale envelope description (\Cref{sec:geophase,sec:effective-field}).
