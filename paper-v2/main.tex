...
\subsection{Parameter-determination package (analytic scaling + numerical closure)}
\label{subsec:parameter_package}

We require parameter choices that reproduce (i) the characteristic propagation speed \(c\),
(ii) an electron-scale frequency \(\omega_C=m_e c^2/\hbar\), (iii) an electron-scale localization length
\(a\sim\lambda_C=\hbar/(m_e c)\), and (iv) a confined, self-consistent nonlinear mode.

\subsubsection{Analytical scaling relations}
Choose a localization scale \(a\) and amplitude ratio \(\chi:=A/a\).
Fix a Poisson ratio \(\nu\) (equivalently \(\lambda/\mu\)) and a pre-stress parameter \(\alpha<1\).
Then small-amplitude calibration provides a relation of the form
\[
c^2=\frac{\mathcal{C}(\alpha,\lambda,\mu)}{\rho_m},
\]
which fixes one modulus-to-density ratio. The remaining absolute scale is fixed by the energy condition
\(\overline{E}=m_e c^2\), which in the weakly nonlinear regime scales as
\[
\overline{E}\;=\;A^2\,\mathcal{E}(\alpha,\nu,a,\omega_C;\rho_m{=}1),
\]
so that \(\rho_m\) (and hence \(\mu,\lambda\)) can be determined by enforcing \(m_e c^2\).

\subsubsection{Numerical recipes (when closed forms do not exist)}
For candidate parameters we compute:
\begin{enumerate}
\item the cycle-averaged excess energy \(\overline{E}\) by quadrature over \(r\) and phase sampling over one period,
\item the Euler--Lagrange residual norm at a fixed phase (e.g.\ \(t=0\)) as a self-consistency diagnostic,
\item (optionally) a coupled spherical ansatz including both transverse and radial responses
(e.g.\ \(X=(R(r)\hat r,w(r,t))\)) to ensure transverse--lateral exchange is not excluded by construction.
\end{enumerate}

All quantities are computed in physical units; discretization is used only for numerical quadrature and does not
introduce microstructure into the constitutive law.

\subsection{Remaining parameter fit as an optimization problem (gradient descent)}
\label{subsec:optimization}

After enforcing the target scales (\(c,\omega_C,a\sim\lambda_C,\overline{E}=m_e c^2\)), the remaining parameters
are determined by minimizing a self-consistency objective. Let \(\vartheta\) denote the free parameter vector
(e.g.\ \(\vartheta=(\alpha,\nu,\varepsilon,a,\ldots)\) or including additional shape parameters).
Define residual fields \(\mathrm{Res}(\vartheta)\) obtained by inserting a trial configuration into the
Euler--Lagrange operator. We minimize an objective of the form
\begin{equation}
J(\vartheta)= w_1\|\mathrm{Res}_w(\vartheta)\|^2 + w_2\|\mathrm{Res}_R(\vartheta)\|^2 + \mathrm{Reg}(\vartheta),
\label{eq:objective}
\end{equation}
where \(\mathrm{Reg}\) enforces physically meaningful bounds (e.g.\ excluding the trivial \(A\to 0\) solution
compensated by unphysical modulus or density scaling). In practice \(J\) is minimized numerically by
gradient-based methods (finite differences or automatic differentiation) or robust derivative-free methods
(Powell/Nelder--Mead) followed by gradient refinement.

% ============================================================
\section{Experimental Settings: Discrete Brane Dynamics in \texorpdfstring{$\mathbb{R}^4$}{R4}}
...
This checklist assumes the following modules and concepts exist in the reference implementation:
\begin{itemize}
...
\subsection{Checklist C: Spatial Berry connection}
In the reference implementation, only the temporal Berry phase is currently computed; the spatial connection below specifies the next required diagnostic.
...
% ============================================================
\section{Falsification Criteria and Failure Modes}
