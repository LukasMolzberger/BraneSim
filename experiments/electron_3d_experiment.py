"""3D Electron Initialization & Simulation (3D brane embedded in 4D).
...
containment. A true long-term confinement equilibrium is a coupled
...
from branesim.electron.electron_initialization import (
    ElectronModeSpec,
...
def main():
    """Run 3D electron experiment (electron initialization + brane evolution)."""
    print("=" * 70)
    print("3D Electron - Realistic Physical Scales")
    print("=" * 70)

    # Initialize test run manager
    run_manager = TestRunManager(experiment_name="electron_3d_experiment")
    ...
    # Configuration - reuse the same Compton-based scaling as photon experiments
    wavelength_phys = constants.lambda_C  # characteristic length scale = λ_C
    ...
    # Choose a radius of ~ one reduced Compton wavelength (in sim units)
    ...
    report = ReportData(
        ...
        symbol_map=[
            ...
            ("radius_sim", r"\(a\)", radius_sim,
             "Electron cavity radius in intrinsic units."),
            ("electron_l", r"\(\ell\)", electron_spec.l,
             "Spherical harmonic degree."),
            ("electron_m", r"\(m\)", electron_spec.m,
             "Spherical harmonic order."),
            ("electron_n", r"\(n\)", electron_spec.n,
             "Radial root index for the Bessel seed."),
            ("electron_amplitude", r"\(A\)", electron_spec.amplitude,
             "Displacement amplitude scale after normalization."),
            ("electron_smooth_edge", r"\(s\)", electron_spec.smooth_edge,
             "Taper softness in the radial window."),
            ("electron_polarization", r"\(\mathbf{p}_1,\mathbf{p}_2\)", electron_spec.polarization,
             "Orthonormal polarization vectors in R^4."),
        ],
        notes=[
    ...
    print(f"  Signal propagation speed c = {constants.c:.6e} m/s")
    print(f"  Domain dimensions: {domain_length_phys_x:.6e} × {domain_length_phys_y:.6e} × {domain_length_phys_z:.6e} m")
    ...
    plt.tight_layout()
    plt.savefig(run_manager.get_plot_path('electron_3d_example_propagation_xy.png'), dpi=150, bbox_inches='tight')
    print(f"  ✓ Saved: electron_3d_example_propagation_xy.png")

    # ========================================================================
    # 2. ORTHOGONAL SLICES (XZ at middle y, YZ at middle x)
    # ========================================================================
    fig2, axes2 = plt.subplots(2, num_snapshots, figsize=(18, 6))
    fig2.suptitle(f'3D Electron - Orthogonal Slices', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(run_manager.get_plot_path('electron_3d_example_orthogonal_slices.png'), dpi=150, bbox_inches='tight')
    print(f"  ✓ Saved: electron_3d_example_orthogonal_slices.png")
    ...
    fig_lat.suptitle(f'3D Electron - Lateral Distortion (XY slice, Color=Direction, Brightness=Magnitude)',
                     fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(run_manager.get_plot_path('electron_3d_example_lateral_distortion.png'), dpi=150, bbox_inches='tight')
    print(f"  ✓ Saved: electron_3d_example_lateral_distortion.png")
    ...
    anim.save(run_manager.get_plot_path('electron_3d_example.mp4'), writer=writer, dpi=100)
    print(f"  ✓ Saved: electron_3d_example.mp4")
    ...
    electron_radius_phys = float(mapper.to_phys_length(electron_spec.radius))
    print(f"  Electron mode: l={electron_spec.l}, m={electron_spec.m}, n={electron_spec.n}")
    print(f"  Electron radius: {electron_radius_phys*1e9:.3f} nm ({electron_radius_phys/constants.lambda_C:.2f} × λ_C)")
    if debug:
    ...
    run_manager.save_config({
        "experiment": "Electron 3D Experiment",
        "device": str(device),
    })
