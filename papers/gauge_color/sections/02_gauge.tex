...
Because the substrate displacement field is real, this bundle is real (its Brillouin-zone curvature
vanishes identically, \Cref{subsec:bz-gauge}), so the $4\pi$ aspect here is a real $\mathbb{Z}_2$
(Zak-type) holonomy of the \emph{linear envelope} --- and the linear envelope is a spin-1 vector, not a
spin-$\tfrac12$ object.
Physical spin-$\tfrac12$ is therefore \emph{not} carried by the envelope; it is an open problem taken up
at the soliton/matter level (Paper~IV), where we expect it as a distinct $\mathbb{Z}_2$ holonomy
($\pi_1(SO(3))=\mathbb{Z}_2$) of the \emph{bound mode's framing relative to the far-field lattice} under a
$2\pi$ rigid rotation. That soliton-layer $\mathbb{Z}_2$ requires only a real, two-sheeted (double-cover)
configuration space and no fundamental complex field.
