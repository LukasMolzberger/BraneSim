...
Solitons in this theory are expected to live in the nonlinearly stabilized regime with
...
\paragraph{Quantization is emergent, not fundamental.}
The discrete content of \eqref{eq:soliton-labels} --- integer or half-integer $J$, discrete parity
$P$, a finite list of $SU(3)$ irreps, integer winding $B$, and the triality-locked $Q_{U(1)}$ ---
is what \emph{quantization} means in this theory.
It is not a postulate of the substrate.
The underlying field is continuous up to the lattice-spacing resolution (Paper~I
\Cref{subsec:continuous-waves}); the discreteness arises entirely from \emph{particle formation}.
Confinement imposes boundary conditions on the continuous field, the self-adjoint eigenproblem of
\Cref{subsec:linearization} on the bounded soliton then has a discrete spectrum, the angular
geometry admits only the integer-labeled VSH multiplets \eqref{eq:vsh-decomposition}, and the
topological winding takes values in $\pi_3(SU(3))=\mathbb{Z}$.
This is the boundary-condition-on-a-continuous-medium mechanism that Paper~I invokes for confined
classical waves (\Cref{subsec:continuous-waves}), here operating at the soliton level.
The familiar quantization of spectra and quantum numbers is therefore a consequence of how
solitonic particles form and are geometrically constrained, not a primitive of the theory.
...
\paragraph{Anti-expansion is the confinement problem.}
The IR instability direction --- the localized mode spreading out and dispersing --- is not a
problem separate from confinement; it is the same confinement question as the kinematic color
confinement of the $SU(3)$ sector (Paper~III \Cref{subsec:color-confinement}), together with the
topological $\pi_3$ texture developed here.
We therefore expect at least two mechanisms to contribute to anti-expansion, and we do not claim
either is sufficient on its own. The geometric quartic supplies the clean Derrick $\lambda^{-1}$
balance below; the $SU(3)$ confinement of the color sector (no free colored asymptotic state,
Paper~III \Cref{subsec:color-confinement}; the $\pi_3(SU(3))$ winding developed here) is expected
to be load-bearing as well, and the two must ultimately be assessed together.

\paragraph{Geometric-quartic contribution to anti-expansion.}
...
All nonlinear runs must use periodic-clamped BC.
...
\begin{enumerate}
  ...
  action, give the lattice-UV anti-collapse argument as a theorem (not just intuition), and treat
  the anti-expansion (IR) direction as the confinement problem of Paper~IV --- combining the
  geometric-quartic balance \eqref{eq:derrick-balance} with the $SU(3)$ confinement mechanism,
  rather than the quartic alone --- tied to the effective field manifold of Paper~III.
