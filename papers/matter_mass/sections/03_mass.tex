...
The proposal that rest mass is field or wave self-energy rather than an intrinsic attribute has a
long pedigree.
The electromagnetic-mass programme of Thomson, Heaviside, Abraham, Lorentz, and
Poincar\'{e}~\citep{Thomson1881,Heaviside1889,Abraham1902,Lorentz1904,Poincare1900} derived the
inertia of the electron from the energy and momentum of its own field, and Einstein's
inertia--energy relation~\citep{Einstein1905inertia} established that the inertia of any body
equals its rest energy divided by $c^2$.
A modern wave-mechanical version is due to Chang~\citep{Chang2018}: particles as confined vacuum
excitations with dispersion $\omega^2=(k^2+l^2)c^2$ and rest mass set by a transverse confinement
wavenumber, $m_0=\hbar l/c$.
The present construction realizes the same idea in a concrete elastic substrate, with $l$ the
inverse equilibrium loop radius $l=1/R^*$, so that $m_0=\hbar l/c$ coincides with the Compton
closure $R^*=\hbar/mc$ of \Cref{subsec:mass-normalization}.
...
\paragraph{Inertial mass and the classical $4/3$ caveat.}
Equation~\eqref{eq:mass-energy} is an energy definition of mass; a consistent theory must
reproduce the inertial mass --- the substrate momentum carried per unit drift velocity,
$p=mv$ --- with the same $m$.
That a traveling excitation of an elastic masses-and-springs medium carries real momentum
proportional to its energy is established generally by Peskin~\citep{Peskin2010}, who finds
$\bar{\mathcal{P}}=\bar{\mathcal{E}}/c$ for acoustic waves as a second-order effect of the
density--velocity correlation; the same mechanism gives the inertia of the confined toroidal
mode here.
The classical electromagnetic-mass programme is a cautionary precedent: weighing only the field
energy of a charged sphere gives an inertial mass $\tfrac{4}{3}U/c^2$ that disagrees with the
energy mass $U/c^2$ (the ``$4/3$ problem''), because a pure field lump is unstable and constitutes
an open subsystem.
The discrepancy vanishes only when the non-electromagnetic binding stresses that hold the lump
together are included in the stress--energy tensor, restoring a proper four-vector and
$m_{\mathrm{inertial}}=E_{\mathrm{rest}}/c^2$~\citep{Poincare1900,Einstein1905inertia,Hecht2009,Brown2007}.
In the present model the geometric quartic anti-expansion ($\propto k_s\alpha/a$) and the time-link
binding compression \eqref{eq:time-link-compression} play the role of Poincar\'{e} stresses.
Open problem~1 below is therefore also the $4/3$ test: the explicit toroidal energy integral must
include these stresses so that $E(R^*)=mc^2$ and the moving-soliton momentum yield a single $m$.
...
\begin{enumerate}
  ...
  The integral must include the geometric quartic and time-link binding stresses so that the
  resulting $E(R^*)=mc^2$ and the moving-soliton momentum $p=mv$ yield the same $m$ (the $4/3$
  consistency test).
