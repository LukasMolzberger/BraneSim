...
For technical work that does not require explicit 4D treatment --- the linear-regime dispersion of \Cref{sec:waves}, the gauge-sector diagnostics developed in the gauge bridge companion paper, and the localized-mode analysis of the matter bridge companion paper --- it is convenient to slice the brane perpendicular to the timelike direction.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.82\textwidth]{diagrams/lattice}
  \caption{%
    The 4D brane lattice embedded in 4D Euclidean ambient space.
    Each spacelike slice (horizontal planes, labeled by $X_4$) is a 3D cubic lattice with axial-only spacelike links (blue).
    Red links connect adjacent slices along the timelike direction $X_4$; the timelike lattice direction is selected by the action's Lorentzian sign structure, not by the ambient geometry.
    Inset: the 7-site local stencil at the highlighted node, consisting of its six spacelike axial neighbors and two temporal neighbors.
  }
  \label{fig:lattice}
\end{figure}
...
This anisotropy is not retuned away; it is the structural feature that the gauge-sector emergence story developed in the gauge bridge companion paper ultimately rests on.

\paragraph{Dual-observer framework: lab anisotropy vs internal isotropy.}
\label{para:dual-observer}
The lab-frame anisotropy above sets up a distinction that the Lorentz bridge companion paper develops in full.
The lab (outside) observer knows the lattice and resolves direction-dependent wave speeds.
The internal (soliton) observer is built from substrate excitations, so its rods, clocks, and signal cones are
themselves substrate-emergent and renormalize with the same direction-dependent local wave speed.
Whether this renders their operationally measured speed of light direction-independent --- and thereby yields
Lorentz-like kinematics for internal observers despite manifest lab-frame anisotropy --- is the observer-universality
argument derived in the Lorentz bridge.
This core paper exports only the setup; the closing mechanism and its decisive empirical test (whether two solitons in
different orientations report the same effective metric) belong to that companion paper.

\paragraph{Brillouin zone.}
The lattice spacing $a$ defines a Brillouin zone $k_i\in[-\pi/a,\pi/a]$ for the substrate eigenmodes.
The geometric-phase diagnostics this enables on the $\mathbf{k}$-space eigenbundles are developed in the gauge bridge
companion paper.
...
\begin{equation}
  ...
  = k_s\!\sum_{\delta\in\mathcal{N}_s}
    ...
    \,\frac{R_{p+\delta}^l - R_p^l}{\bigl|R_{p+\delta}^l - R_p^l\bigr|},
  \label{eq:el-verlet}
\end{equation}
where the final factor is the unit vector along the link; the $1/\lvert R_{p+\delta}^l-R_p^l\rvert$ is the central-force normalization that follows from differentiating the scalar link length in \eqref{eq:slice-potential}.
...
\end{equation}
This is the energy the simulation kernel computes exactly.
...
to a gradient expansion $W = W_2 + W_4 + \cdots$.
...
\end{equation}
in exact agreement with the lattice closed-form dispersion
...
are indistinguishable in the linear-elasticity regime of \Cref{sec:waves}.
...
\emph{Quantitative soliton sizing (matter bridge companion paper) must use the spring law
...
For the linear and moderate-strain results of \Cref{sec:waves}
this is immaterial; for the strongly localized modes of the matter bridge companion paper it must be
