...
(\Cref{eq:eom} of Paper~I) on a single 4D world-volume: each physical history is one definite
...
carries no quantization of its own (Paper~I \Cref{subsec:continuous-waves}, Paper~III
\Cref{subsec:spinorial-holonomy}); the discrete level structure that quantizes spectra belongs to
the confined matter solitons (Paper~IV). The quantization and the apparent randomness one
...
A common objection is that retrocausality is not the \emph{only} option: one could instead keep
(D) and (MI) and give up (L), as pilot-wave / Bohmian mechanics --- and indeed most realist
completions of quantum mechanics --- do.
This is the one move the substrate cannot make.
Locality is not an assumption we are free to trade away; it is a structural fact of a
nearest-neighbor elastic lattice with finite signal speed (\Cref{subsec:which-assumption}).
Every nonlocal completion is therefore excluded on that single ground, with Bohmian mechanics only
the most prominent representative; the Bohmian case carries a further structural conflict --- it
requires a preferred spacelike foliation and a nonlocal guidance equation, both alien to the
covariant 4D worldtube --- but it is already ruled out before that conflict is reached.
Laid out as a complete enumeration for a locally-interacting 4D brane lattice, the options are:
\begin{enumerate}
