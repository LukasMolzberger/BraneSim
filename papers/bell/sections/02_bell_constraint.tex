...
\begin{itemize}
  \item \textbf{(D) cannot be given up.} The equations of motion (\Cref{eq:brane-eom}) are
  ...
  \item \textbf{(L) cannot be given up.} The substrate forces are nearest-neighbor spring contacts.
  ...
  Throughout, (L) denotes this \emph{dynamical} (Bell) locality --- finite-speed influence in the
  equations of motion; it is distinct from operational no-signalling, which the model
  \emph{derives} as an output (\Cref{subsec:born-correlations}) rather than assumes.
\end{itemize}
...
the confined matter solitons (\Cref{sec:substrate}). The quantization and the apparent randomness one
