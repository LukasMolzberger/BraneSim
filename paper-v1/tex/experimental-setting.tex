...
\item \textbf{Electromagnetism (geometric phase, exploratory):} Minimal $U(1)$ Berry phase / holonomy readout from prepared narrowband complex envelopes derived from $(\mathbf X,\partial_t\mathbf X)$; full field reconstruction is deferred.
...
\item \textbf{Spinor behavior (future):} Non-Abelian holonomy in degenerate subspaces is a plausible mechanism for $4\pi$ transport, but is deferred in the present step-by-step revision.
\end{itemize}
...
A key methodological commitment is that geometric phase readout is \emph{not created}
...
\emph{substrate initialization}. Any optional FFT masks or branch projections are used
only as secondary sanity checks and visualization aids.
...
\paragraph{Electron packet (double-winding strip in a torus).}
For the electron-like defect we initialize a narrowband carrier confined to a tubular neighborhood of a
closed centerline $\mathbf{C}(z)$ (Appendix~\ref{app:tubular-electron}), with a double-ridge
(double-winding) cross-section inspired by Williamson--van der Mark. In implementation, the geometry
is generated by the helper code (e.g.\ \texttt{tubular\_electron\_geometry.py}) that provides the
centerline, tubular frame, and ridge loci.
Along the loop coordinate $z$ we impose a \emph{half-angle} rotation of the internal polarization
frame so that one circuit produces a sign flip (spinorial $4\pi$ periodicity), while two circuits
return the state.

The central question is whether the substrate admits a long-lived \emph{self-guided} circulating state
without adding an external confinement potential. In this step-by-step revision we therefore focus on
(i) preparing a clean narrowband carrier, (ii) testing stability of the toroidal configuration, and
(iii) extracting only the minimal geometric-phase quantities needed for later gauge identification.
Non-Abelian (Wilczek--Zee) transport is a natural next step but is deferred here.

\subsubsection{Confinement characterization in a radial slice}
To connect the toroidal geometry to the required bending strength, we analyse a static ``radial slice''
through the tube cross-section: for a fixed point on the centerline, consider the line $r\ge 0$
emanating outward in the brane and evaluate background deformation, stress, and effective propagation
characteristics along it.
A key conceptual caveat is that an effective index profile $n_{\rm eff}(r)$ is not a geometric
property of the embedding alone; it is defined by the dispersion relation of small fluctuations
linearized about the background configuration (Appendix~\ref{app:confinement-slice}).
In practice we extract from simulations:
(i) a background state by local time averaging over carrier periods,
(ii) a stress proxy $P^{ss}(r)=2\,\partial W/\partial g_{ss}$, and
(iii) an effective wave-speed proxy $c_{\rm eff}(r)$ for the intended carrier branch (either by local
linearization or via the tension-dominated estimate $c_{\rm eff}^2\propto P^{ss}/\rho_m$).
We then form $n_{\rm eff}(r)=c_{\rm ref}/c_{\rm eff}(r)$ and check two order-of-magnitude constraints:
(a) bending/guiding requires $\Delta n/n\sim a/R$ for tube radius $a$ and ring radius $R$, and
(b) inner/outer ridge coherence implies $\Delta k/k\sim 2w_0/R$ at fixed temporal frequency, with
ridge offset $w_0$ (Appendix~\ref{app:confinement-slice}).

\subsubsection{Minimal geometric-phase readout (U(1))}
In the present step-by-step revision we restrict attention to prepared narrowband packets
(Section~\ref{sec:init-carriers}) and do \emph{not} rely on a posteriori spectral band-pass masks
or dispersion-branch projections to define the relevant complex mode.
Instead, we construct a complex envelope $\varphi(x,t)\in\mathbb C^4$ from the real phase-space pair
$(\mathbf u(x,t),\mathbf v(x,t))$ by a consistent quadrature (analytic-signal) representation
of the prepared carrier, and normalize $\tilde \varphi=\varphi/\|\varphi\|$.

The single-band ($U(1)$) geometric phase is meaningful only when the packet effectively occupies
an isolated branch on its spectral support. In practice we verify this by checking that (i) the
initialized spectrum is narrow and (ii) nearby branches remain separated by a minimum gap over the
packet support for the chosen parameters.

For numerical holonomy we use the gauge-invariant overlap formulation: for a discrete step
$R\to R'$ in spacetime,
\begin{equation}
  U(R\to R') :=
...
\end{equation}
and the loop phase is $\Theta[\gamma]=\arg\prod_{(R\to R')\in\gamma} U(R\to R')$.
We do not yet attempt curvature maps, defect/source reconstruction, or a full identification
with Maxwell theory in this paper revision.
...
A future goal is to test whether geometric phase readout of prepared narrowband modes can be
consistently mapped to an effective electromagnetic sector. Since this revision deliberately
avoids full field reconstruction (effective $A_\mu$, $F_{\mu\nu}$, and source localization),
we do not attempt a charge calibration here. Charge matching is postponed until the minimal
holonomy measurements are numerically stable and reproducible.
...
All tests are formulated as measurements on simulation outputs. No derived
...
\item \textbf{Preparation sanity (narrowband by construction):}
Verify that the initialized packet is spectrally narrow on its support (by direct FFT inspection as a sanity check),
so that the subsequent geometric-phase readout does not require diagnostic band-pass filtering.

\item \textbf{Minimal holonomy sanity (U(1)):}
Compute loop phases using the overlap formulation and confirm that trivial loops yield vanishing phase within numerical tolerance.
\end{enumerate}

\subsection{Implementation and interpretation notes}
