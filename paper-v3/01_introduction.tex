...
(i) a single substrate with local action dynamics (posited microscopically as a cubic lattice embedded in $\R^4$, with a continuum long-wavelength effective description),
...
develops transverse structure $X^4=u(x,t)$, then the induced metric gains a quadratic contribution
$\partial_i u\,\partial_j u$; because the in-brane displacement is dynamical, a pre-tensioned medium relaxes by
generating an inward-biased lateral response rather than only a local stress increase. Coarse-grained over scales large compared to the carrier wavelength, such
...
polarization branch).
On a lattice substrate, band structure provides a natural setting for narrowband sectors; here we treat the
existence of an ordered carrier sector as a working postulate and focus on its diagnostic consequences.
...
The microphysical lattice postulate and its parameterization are stated in \Cref{subsec:lattice-substrate};
geometric-phase diagnostics are developed in \Cref{sec:geophase}.
