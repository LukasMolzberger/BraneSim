...
To make this explicit, consider the simple near-identity embedding in which the first three components follow the
material coordinates and the fourth component is a transverse displacement,
\begin{equation}
  \vecX(x,t)\approx (x^1,x^2,x^3,u(x,t)).
  \label{eq:graph-embedding}
\end{equation}
Then the induced metric becomes
\begin{equation}
  g_{ij}=h_\star^2\,\delta_{ij}+\partial_i u\,\partial_j u,
  \label{eq:metric-graph}
\end{equation}
where $h_\star$ denotes the held ground-state scale introduced in \Cref{eq:alpha-def} (equivalently, one may rescale coordinates so that $h_\star=1$),
a large-amplitude transverse profile necessarily produces an \emph{in-brane} strain contribution of order
$|\nabla u|^2$ even if the lateral components remain close to their ground state.
...
program: localized transverse gradients can act as sources of slowly varying background strain/dilation, and linear
wave packets propagating on top of that background experience branch-dependent effective coefficients
...
Deriving such a reduction quantitatively is an open problem of the present theory fragment.
