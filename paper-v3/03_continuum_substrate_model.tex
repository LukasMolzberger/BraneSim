...
\subsection{Cubic lattice substrate and the continuum limit}
...
Interactions are encoded by springs/couplings between neighboring sites (optionally including diagonals to improve
effective isotropy at long wavelength).
...
\end{equation}
where $h_\star$ denotes the held ground-state scale introduced in \Cref{eq:alpha-def}
(equivalently, one may rescale coordinates so that $h_\star=1$).
