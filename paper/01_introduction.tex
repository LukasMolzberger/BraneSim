...
This paper is a \emph{substrate-consistency} argument, not a narrow numerical result.
Its core claim is that a single minimal ontological model --- the 4D brane lattice --- can be connected, without contradiction, to effective Lorentz kinematics, gauge-like transport, a gravity-like contraction channel, localized matter, and a Bell-compatible interpretation.
Each connection is stated as an explicit mathematical bridge that can be attacked by calculation.
The breadth is therefore not speculative overreach but a structural requirement: if any bridge fails, the substrate hypothesis fails.
The intended contribution is to make the architecture explicit enough that these failures (or successes) are decidable by computation.

\paragraph{Paper organization.}
\Cref{sec:scope} states the consistency architecture: the five bridges, the first-layer objections each bridge addresses, the non-claims, and the concrete numerical program.
\Cref{sec:continuum-model} formulates the substrate model, making the geometric source of the nonlinearity explicit.
\Cref{sec:emergent-relativity} develops the Lorentz bridge.
\Cref{sec:geophase,sec:effective-field} develop the gauge bridge via per-wavepacket band isolation and Berry/Wilczek--Zee transport.
\Cref{sec:localized} develops the localized-matter bridge as a self-adjoint eigenproblem with nonlinear self-guidance.
\Cref{sec:bell-interpretation} states the Bell bridge: a retrocausal worldtube reading in which causality emerges from soliton chirality.
\Cref{sec:discussion} situates the framework against existing emergent-gauge constructions and identifies the gravity bridge's open quantitative problem.
