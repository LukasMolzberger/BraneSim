...
Using the per-wavepacket band isolation of \Cref{subsec:per-wavepacket-isolation}, write the substrate
displacement as a slowly varying complex envelope multiplied by a local mode frame:
\begin{equation}
  \vecu(x,t)\;\approx\;
  \mathrm{Re}\!\left[
...
$\pi_1(U(1))=\mathbb{Z}$ admits line vortex defects (phase winds by $2\pi$ around the core
\citep{NielsenOlesen1973,HindmarshKibble1995CosmicStrings});
$\pi_1(SU(3))=0$ forbids vortices; and $\pi_3(SU(3))=\mathbb{Z}$ admits Skyrmion/instanton-type textures
\citep{Skyrme1962,AtiyahManton1989}.
The two sectors therefore support topologically \emph{different} classes of
...
\paragraph{Interaction of the two sectors.}
% Internal derivation note: paper/derivations/u1_su3_binding.md
The decomposition in \Cref{eq:u3-decomp} is additive at the Lie-algebra level, but the two sectors are not dynamically independent.
At quadratic (linear) order the envelope gradient energy is block-diagonal in trace/traceless components, so the sectors decouple exactly; any coupling is necessarily nonlinear.
A group-theoretic tie is however exact and dynamics-free: because the field is $U(3)=\bigl(U(1)\times SU(3)\bigr)/\mathbb{Z}_3$ --- the center $\mathbb{Z}_3\subset SU(3)$ identified with the cube-roots of unity in $U(1)$ --- single-valuedness forces the trace charge $q$ and the $SU(3)$ triality $t\in\{0,1,2\}$ to satisfy $q\equiv -t\pmod{3}$, the standard representation-theory relation for $U(N)=\bigl(U(1)\times SU(N)\bigr)/\mathbb{Z}_N$ \citep{GeorgiGlashow1974,BaezHuerta2010GUTAlgebra}; hence a color triplet carries fractional ($\tfrac{1}{3}$-integer) trace charge while a color singlet carries integer charge, and charge fractionality is locked to color representation without reference to dynamics.
At the model-specific level, the central-force anharmonicity --- the Euclidean-norm term, exactly $\propto\alpha$ (see \Cref{subsec:constitutive-law,eq:W4}) --- couples the sectors at quartic order through a vertex $\propto\alpha|\Psi_{\mathrm{tr}}|^2|\Psi_\perp|^2$, which vanishes identically as $\alpha\to 0$; the prestress parameter that splits the two sectors' spectral scales is therefore also the parameter that couples them energetically.
Whether this coupling dynamically co-locates the charge with the color core --- so that a charged baryon binds rather than fragmenting --- is a target of the numerical program (\texttt{OPEN\_PROBLEMS.md} D6 / \texttt{EXPERIMENT.md}), not a conclusion of the present paper.
...
in the present theory that mixture is reinterpreted as follows:
\begin{itemize}
