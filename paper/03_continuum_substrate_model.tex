...
\end{equation}
identical in form to older 3D-in-4D formulations.
The fourth ambient component, denoted $u$ in this slicing, is the inside observer's chosen timelike component of $\vecX$ --- what earlier formulations called the ``amplitude'' direction.
...
length.
Link contributions along the timelike direction enter the full brane action with opposite sign and are absorbed into the kinetic term $\tfrac12\rho_m|\partial_t\vecX|^2$ of the slice Lagrangian (\Cref{eq:lagrangian-density} below); their explicit lattice form is a separate design question and is not required for any spacelike-slice analysis in this paper.
