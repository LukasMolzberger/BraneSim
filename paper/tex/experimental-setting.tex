...
Given the normalized envelope $\tilde u(x,t)$, we compute the connection
one-form directly via Eq.~\eqref{eq:spacetime-berry-connection}:
\begin{equation}
a_\mu(x,t) = i \sum_{A=1}^{4} \tilde u^{A\ast}(x,t) \, \partial_\mu \tilde u^A(x,t),
\end{equation}
where $\partial_\mu = (\tfrac{1}{c}\partial_t, \nabla)$. Spatial derivatives
are computed via central finite differences on the discrete grid; temporal
derivatives are computed from consecutive snapshots.

The curvature two-form follows from Eq.~\eqref{eq:spacetime-berry-curvature}:
\begin{equation}
f_{\mu\nu}(x,t) = \partial_\mu a_\nu - \partial_\nu a_\mu.
\end{equation}

This curvature is gauge-invariant and measures the local holonomy rate. Regions
of nonzero $f_{\mu\nu}$ indicate geometric obstruction to globally trivial phase
choice.
