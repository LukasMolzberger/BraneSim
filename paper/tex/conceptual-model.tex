...
analogous rule:
\emph{apply any time--frequency or $(\mathbf{k},\omega)$ filtering to the single-valued
substrate fields first, and only then project onto the target band/subspace.}
...
substrate's configuration space, isolated by one of the following procedures:
\begin{enumerate}
\item \textbf{Dispersion-branch isolation:} For a locally translation-invariant
...
\item \textbf{Wavepacket filtering:} Select a region $\mathcal B\subset
...
\item \textbf{Demodulation + low-pass filter:} Multiply the substrate field by
  a carrier $e^{i(\mathbf k_0\cdot\mathbf x-\omega_0 t)}$ and apply a spatial
  and temporal low-pass filter to extract the slowly varying envelope.
\end{enumerate}
