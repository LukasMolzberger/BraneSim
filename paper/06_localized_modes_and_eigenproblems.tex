...
\Cref{fig:seed-snapshot} illustrates the VSH construction protocol for such a seed on the $112^3$ lattice.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{diagrams/exp_seed_snapshot}
  \caption{Constructed $Y_1^1$ $U(1)$-trace vortex candidate seed on the $112^3$ lattice (solver-untouched initial condition, iteration~0).
  \textit{Left:} XZ midplane, carrier phase encoded as RGB (ring cross-section --- two lobes, $\pi$ out of phase).
  \textit{Centre:} XY midplane plan view, carrier phase as RGB (azimuthal $m{=}1$ winding visible as a full colour cycle around the ring).
  \textit{Right:} XZ midplane amplitude; the $\sin\theta$ envelope of the $Y_1^1$ harmonic produces the donut cross-section with a dark axis.
  This panel illustrates the VSH construction protocol (\Cref{sec:angular-structure}): the seed is a programmatic ansatz, not a demonstrated stable solution.
  Which objects the substrate dynamics actually stabilizes remains open (\Cref{sec:scope}, non-claims; \texttt{OPEN\_PROBLEMS.md} D4).}
