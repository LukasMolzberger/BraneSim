...
\Cref{fig:color-channels} shows the numerical apparatus that performs this decomposition on the seeded lateral triplet.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{diagrams/exp_color_channels}
  \caption{$U(1)$-trace / $SU(3)$-traceless channel decomposition of a seeded lateral triplet (diagnostic D6 of the numerical apparatus).
  \textit{Top row, left:} per-channel lateral energy over time; for the pure-trace $Y_1^1$ seed the $U(1)$ trace channel carries essentially all energy.
  \textit{Top row, centre:} channel-energy fraction ($U(1) + SU(3) = 1$, conserved); the $U(1)$ fraction reads ${\approx}1$, confirming the seed is correctly constructed as a pure-trace object.
  \textit{Top row, right:} per-channel displacement amplitude over time.
  \textit{Bottom row:} XY midplane amplitude maps of the $U(1)$ trace channel (left, donut ring) and the $SU(3)$ traceless channel (centre, consistent with near-zero amplitude for a pure-trace seed); per-lateral-component displacement as a function of time (right).
  This figure validates the decomposition diagnostic for a pure-trace seed; it does \emph{not} claim that a dynamical simulation produces a mixed or bound $U(1)$--$SU(3)$ state (a question left open by \Cref{sec:scope}).}
