...
\subsection{\texorpdfstring{Axis-aligned triplet phases and coupling-induced mixing ($U(1)^3\rightarrow U(3)$)}{Axis-aligned triplet phases and coupling-induced mixing (U(1)^3 -> U(3))}}
\label{subsec:axis-triplet}

On a cubic lattice substrate it is natural to organize the per-wavepacket complex envelope as a three-component triplet
...
In the lattice substrate, $g_{\mathrm{mix}}$ is an effective measure determined by the prestretch $\alpha$ and by
the microscopic coupling stencil (notably off-axis/diagonal couplings).
...
For an approximately threefold-degenerate retained subspace, one obtains a $U(3)$ connection
